...
<head>
  ...
  <?php
    ...
    $quanly = new QuanLy();
    $result = $quanly->getthongTinNhanVienById($idAccount);
    $id =$result['id'];
    $ho = $result['ho'];
    $ten = $result['ten'];
    $ngays = $result['ngaySinh'];
    $gioi = $result['gioi'];
    ...
    $infor = '';
    $object = [];
    if(isset($_POST['id']) &&  isset($_POST['ho']) && isset($_POST['ten']) && isset($_POST['ngaysinh']) && isset($_POST['gioi']) && isset($_POST['userName'])&& isset($_POST['password'])&& isset($_POST['type']) && isset($_POST['submit'])){
        ...
        if (empty($id)) {
          ...
          // Sửa đổi thông tin tài khoản
            ...
            $object['pass'] = $pass;
            $object['type'] = $type;
            $result = $quanly->chinhSuaTaiKhoan($object);
           if ($result['code'] == 0){
              $check1 = true;
...
<body>
  <nav class="navbar navbar-expand-sm navbar-infor">
    ...
    <button class="create-btn" type="submit" name="submit">Sửa tài khoản</button>
